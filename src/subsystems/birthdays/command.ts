...
export default {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("set")
        .setDescription("Set a new birthday. Will override the previous.")
        ...
        .addIntegerOption((option) =>
          option
            ...
            .setRequired(true)
            .setMinValue(new Date(Date.now()).getUTCFullYear() - 100)
            .setMaxValue(new Date(Date.now()).getUTCFullYear())
        )
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("get").setDescription("View your current birthday.")
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("remove")
    ...
    switch (interaction.options.getSubcommand()) {
      case "set": {
        ...
        replySuccess(
          interaction,
          this.ephemeral,
          `Your birthday record has been set to: ${date.format("DD MMMM YYYY")}`
        );
        break;
      }
      ...
      case "get": {
        ...
        if (birthday == undefined) {
          replyFailure(interaction, "You do not have a birthday on record.");
        } else {
          replySuccess(
            interaction,
            this.ephemeral,
            `Your birthday day on record is: ${dayjs(birthday).format(
              "DD MMMM YYYY"
            )}`
          );
        }
